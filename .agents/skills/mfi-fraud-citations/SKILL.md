...
This skill is the canonical mapping of the 17 fraud-detection signals in the Medicaid Inspector app to their legal authority. The Python codebase (`backend/services/narrative_generator.py`) embeds the same data inside the case-narrative templates; this Skill exposes it for Codex to cite in conversation without needing the source file open.
