...
async def provider_oig_tip(npi: str, destination: str = "oig", save_note: bool = True):
    ...
    # ── Persist the draft to the case record (Dave's rule, 2026-07-25) ────────
    # A generated narrative used to exist ONLY in the caller's chat/modal: HAL
    # would say "drafted and ready" but nothing was saved, so there was nothing
    # to go back to. Every generation — from HAL's draft_oig_tip OR the app's
    # OIG Tip button — now lands on the case's permanent append-only note log.
    # Best-effort and de-duplicated; never fails the generation.
    saved_to_case = False
    save_skipped = ""
    if save_note:
        try:
            from core.review_store import add_case_note, get_case_notes, MAX_CASE_NOTE_CHARS
            marker = f"[{dest.upper()} REFERRAL NARRATIVE - generated by MFI]"
            existing = get_case_notes(npi)
            body = f"{marker}\n\n{text_with_guide if guide_block else text}"
            # De-dup on CONTENT, not just the marker. Marker-only matching meant a
            # case that got an early draft could never receive an improved one —
            # e.g. narratives saved before the per-provider filing guide existed
            # were frozen without it. The log is append-only, so a materially
            # different regeneration is appended (superseding, history intact).
            def _norm(s: str) -> str:
                return " ".join((s or "").split())
            # Trim FIRST so the dedup compares what actually gets stored —
            # otherwise a truncated note never matches and re-saves every call.
            ...
            _new = _norm(body)
            if existing is None:
                save_skipped = ("NPI is not in the review queue — add it to a case to keep "
                                "drafts on the record")
            elif any(_norm(n.get("text") or "") == _new for n in existing):
                save_skipped = "an identical draft is already on this case's note log"
            else:
                saved_to_case = add_case_note(
                    npi, body, actor="mfi:oig-tip", actor_type="ai") is not None
        except Exception:
            import logging as _logging
            _logging.getLogger(__name__).warning(
                "oig-tip: case-note save failed for %s", npi, exc_info=True)
            save_skipped = "case-note write failed (see server log)"
