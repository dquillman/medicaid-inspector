...
def _save_rule_to_firestore(rule: dict) -> None:
    """Best-effort write of a single rule to Firestore."""
    try:
        from core.firestore_store import save_alert_rule
        save_alert_rule(rule)
    except Exception as e:
        logger.warning(f"[alert_store] Firestore write failed (non-fatal): {e}")


def _delete_rule_from_firestore(rule_id: str) -> None:
    """Best-effort delete of a rule from Firestore."""
    try:
        from core.firestore_store import delete_alert_rule
        delete_alert_rule(rule_id)
    except Exception as e:
        logger.warning(f"[alert_store] Firestore delete failed (non-fatal): {e}")


def load_rules_from_disk() -> None:
    """Load rules — try Firestore first, fall back to JSON file."""
    global _rules
    # Try Firestore first
    try:
        from core.firestore_store import load_alert_rules
        fs_rules = load_alert_rules()
        if fs_rules:
            _rules = {r["id"]: r for r in fs_rules}
            print(f"[alert_store] Loaded {len(_rules)} alert rules from Firestore")
            return
    except Exception as e:
        logger.warning(f"[alert_store] Firestore load failed, falling back to JSON: {e}")
...
def save_rules_to_disk() -> None:
    """Dual-write: save to JSON file AND sync all rules to Firestore."""
    # Always write JSON (fallback)
    try:
        atomic_write_json(_RULES_FILE, {"rules": list(_rules.values())})
    except Exception as e:
        print(f"[alert_store] Could not save alert rules to disk: {e}")

    # Best-effort Firestore sync
    for rule in _rules.values():
        _save_rule_to_firestore(rule)
...
def add_rule(rule: dict) -> dict:
    ...
    # Dual-write: JSON + Firestore
    save_rules_to_disk()
    _save_rule_to_firestore(new_rule)
    return new_rule
...
def delete_rule(rule_id: str) -> bool:
    ...
    # Dual-write: JSON + Firestore
    save_rules_to_disk()
    _delete_rule_from_firestore(rule_id)
    return True
