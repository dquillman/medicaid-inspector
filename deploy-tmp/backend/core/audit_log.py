...
def load_audit_from_disk() -> None:
    """Load audit entries — try Firestore first, fall back to JSON file."""
    global _entries, _next_id

    # Try Firestore first
    try:
        from core.firestore_store import load_audit_entries
        fs_entries = load_audit_entries(limit=5000)
        if fs_entries:
            _entries = fs_entries
            if _entries:
                _next_id = max(e.get("id", 0) for e in _entries) + 1
            print(f"[audit_log] Loaded {len(_entries)} audit entries from Firestore")
            return
    except Exception as e:
        logger.warning(f"[audit_log] Firestore load failed, falling back to JSON: {e}")
...
def _save_entry_to_firestore(entry: dict) -> None:
    """Best-effort write of a single audit entry to Firestore."""
    try:
        from core.firestore_store import append_audit_entry
        append_audit_entry(entry)
    except Exception as e:
        logger.warning(f"[audit_log] Firestore write failed (non-fatal): {e}")
