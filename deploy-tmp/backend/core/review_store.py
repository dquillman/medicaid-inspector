...
def _save_item_to_firestore(item: dict) -> None:
    """Best-effort write of a single review item to Firestore."""
    try:
        from core.firestore_store import save_review_item
        save_review_item(item)
    except Exception as e:
        logger.warning(f"[review_store] Firestore write failed (non-fatal): {e}")


def load_review_from_disk() -> None:
    """Load review queue — try Firestore first, fall back to JSON file."""
    global _review_items

    # Try Firestore first
    try:
        from core.firestore_store import load_review_items
        fs_items = load_review_items()
        if fs_items:
            _review_items = {item.get("npi", item.get("id")): item for item in fs_items}
            print(f"[review_store] Loaded {len(_review_items)} review items from Firestore")
            return
    except Exception as e:
        logger.warning(f"[review_store] Firestore load failed, falling back to JSON: {e}")
...
def save_review_to_disk() -> None:
    """Dual-write: save to JSON file AND sync changed items to Firestore."""
    # Always write JSON (fallback)
    try:
        atomic_write_json(_QUEUE_FILE, {"items": list(_review_items.values())})
    except Exception as e:
        print(f"[review_store] Could not save review queue to disk: {e}")
...
def add_to_review_queue(providers: list[dict]) -> int:
    ...
    if added:
        save_review_to_disk()
        # Sync newly added items to Firestore
        for p in providers:
            npi = p.get("npi")
            if npi and npi in _review_items:
                _save_item_to_firestore(_review_items[npi])
    return added
