...
On Cloud Run, the cache is synced to/from GCS so it survives container restarts.
"""
import json
import logging
import os
...
_GCS_BUCKET = os.environ.get("GCS_CACHE_BUCKET", "medicaid-inspector-data")
_GCS_BLOB = "prescan_cache.json"
...
def _download_from_gcs() -> bool:
    """Download prescan cache from GCS if local file is missing. Returns True on success."""
    try:
        ...
        if not blob.exists():
            _log.info("[store] No cache in GCS yet")
            return False
        blob.download_to_filename(str(_CACHE_FILE))
        _log.info(f"[store] Downloaded cache from gs://{_GCS_BUCKET}/{_GCS_BLOB}")
        return True
    except Exception as e:
        _log.warning(f"[store] Could not download cache from GCS: {e}")
        return False


def _upload_to_gcs() -> None:
    """Upload prescan cache to GCS (best-effort, non-blocking)."""
    try:
        from google.cloud import storage
        client = storage.Client()
        bucket = client.bucket(_GCS_BUCKET)
        blob = bucket.blob(_GCS_BLOB)
        blob.upload_from_filename(str(_CACHE_FILE))
        _log.info(f"[store] Uploaded cache to gs://{_GCS_BUCKET}/{_GCS_BLOB}")
    except Exception as e:
        _log.warning(f"[store] Could not upload cache to GCS: {e}")
...
def save_to_disk() -> None:
    ...
    try:
        ...
        # Sync to GCS so cache survives container restarts
        import threading
        threading.Thread(target=_upload_to_gcs, daemon=True).start()
    except Exception as e:
        _log.warning(f"[store] Could not save cache: {e}")
...
def load_from_disk() -> bool:
    ...
    try:
        if not _CACHE_FILE.exists():
            _log.info("[store] No local cache — trying GCS...")
