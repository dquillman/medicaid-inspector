...
from routes import whistleblower
...
from core.pecos_store import load_pecos_from_disk, download_pecos_data
...
async def _run_scan_batch(batch_size: int, state_filter: Optional[str], force: bool = False):
    ...
    from services.anomaly_detector import (
        ...
        beneficiary_sharing_anomaly,
        pecos_enrollment_gap,
        reassignment_chain_depth,
        suppress_unsupported_signals,
    )
    ...
    try:
        ...
        # ── Step 3a: servicing NPI breakdown ──────────────────────────────────
        set_prescan_status(3, f"Fetching servicing NPI data for {len(npi_list)} providers…")
        servicing_by_npi: dict[str, list[dict]] = {}
        try:
            servicing_sql = f"""
            SELECT
            ...
            ORDER BY billing_npi, total_paid DESC
            """
            servicing_rows = await query_async(servicing_sql, npi_params)
            for r in servicing_rows:
                servicing_by_npi.setdefault(r["billing_npi"], []).append(r)
            log.info("Servicing NPI query: %d rows for %d providers", len(servicing_rows), len(servicing_by_npi))
        except Exception as svc_exc:
            log.warning("Servicing NPI query failed (signals proceed without): %s", svc_exc)

        # ── Step 3b: beneficiary sharing (if BENE_ID exists) ──────────────────
        # Two-step approach to avoid expensive full-table self-join:
        #   1. Get distinct BENE_IDs per target NPI
        #   2. Find other NPIs sharing those beneficiaries (limited scope)
        bene_sharing_by_npi: dict[str, list[dict]] = {}
        try:
            bene_sharing_sql = f"""
            WITH target_benes AS (
                SELECT DISTINCT
                    BILLING_PROVIDER_NPI_NUM AS npi,
                    BENE_ID
                FROM read_parquet('{get_parquet_path()}')
            ...
            bene_counts AS (
                SELECT npi, COUNT(DISTINCT BENE_ID) AS provider_total_benes
                FROM target_benes
                GROUP BY npi
            ),
            other_providers AS (
                SELECT DISTINCT
                    BILLING_PROVIDER_NPI_NUM AS other_npi,
                    BENE_ID
                FROM read_parquet('{get_parquet_path()}')
                WHERE BENE_ID IN (SELECT DISTINCT BENE_ID FROM target_benes)
                  AND BILLING_PROVIDER_NPI_NUM NOT IN ({npi_placeholders})
                  AND BENE_ID IS NOT NULL
            ),
            shared AS (
                SELECT
                    t.npi,
                    bc.provider_total_benes,
                    o.other_npi,
                    COUNT(DISTINCT t.BENE_ID) AS shared_bene_count
                FROM target_benes t
                INNER JOIN other_providers o ON t.BENE_ID = o.BENE_ID
                INNER JOIN bene_counts bc ON t.npi = bc.npi
                GROUP BY t.npi, bc.provider_total_benes, o.other_npi
                HAVING shared_bene_count >= 5
            )
            SELECT npi, other_npi, shared_bene_count, provider_total_benes
            FROM shared
            ORDER BY npi, shared_bene_count DESC
            """
            bene_rows = await query_async(bene_sharing_sql, (*npi_params, *npi_params))
            for r in bene_rows:
                bene_sharing_by_npi.setdefault(r["npi"], []).append(r)
            log.info("Beneficiary sharing query: %d rows for %d providers", len(bene_rows), len(bene_sharing_by_npi))
        except Exception as bene_exc:
            # BENE_ID column may not exist in this dataset — silently continue
            log.info("Beneficiary sharing query skipped (BENE_ID not available or error): %s", bene_exc)

        # ── Step 3.5: NPPES pre-fetch for current batch ────────────────────
        # Fetch NPPES identity data BEFORE scoring so all 17 signals have
        # access to taxonomy, address, status, enumeration_date, and
        # authorized_official data on first scan (not just on rescan).
        set_prescan_status(3, f"Fetching NPPES identity for {len(npi_list)} providers…")
        nppes_by_npi: dict[str, dict] = {}
        try:
            from data.nppes_client import get_provider as _nppes_get
            _nppes_sem = asyncio.Semaphore(15)

            async def _fetch_nppes(npi: str):
                async with _nppes_sem:
                    try:
                        return npi, await _nppes_get(npi)
                    except Exception:
                        return npi, {}

            _nppes_pairs = await asyncio.gather(*[_fetch_nppes(n) for n in npi_list])
            nppes_by_npi = {npi: data for npi, data in _nppes_pairs if data}
            log.info("NPPES pre-fetch: %d/%d providers enriched", len(nppes_by_npi), len(npi_list))
        except Exception as nppes_exc:
            log.warning("NPPES pre-fetch failed (scoring proceeds without): %s", nppes_exc)

        # Inject NPPES data into each aggregate row before scoring
        for row in agg_rows:
            nppes_data = nppes_by_npi.get(row["npi"])
            if nppes_data:
                row["nppes"] = nppes_data
        ...
        # Pre-compute cluster sizes from prescan cache + current batch NPPES data
        cluster_sizes = compute_address_clusters(extra_providers=agg_rows)
        auth_clusters = compute_auth_official_clusters(extra_providers=agg_rows)
        ...
        for row in agg_rows:
            ...
            s18 = ghost_employee_billing(npi, servicing_by_npi.get(npi, []))
            s19 = servicing_npi_concentration(npi, servicing_by_npi.get(npi, []))
            s20 = beneficiary_sharing_anomaly(npi, bene_sharing_by_npi.get(npi, []))
            s21 = pecos_enrollment_gap(npi)
            s22 = reassignment_chain_depth(npi)

            signals = [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, s16, s17, s18, s19, s20, s21, s22]
            signals = suppress_unsupported_signals(signals)
        ...
        # Auto-evaluate custom alert rules against freshly-scanned providers
        try:
            from core.alert_store import evaluate_rules, get_rules
            from core.audit_log import log_action
            all_providers = get_prescanned()
            enabled_rules = [r for r in get_rules() if r.get("enabled")]
            if enabled_rules and all_providers:
                alert_results = evaluate_rules(all_providers)
                total_matches = sum(r["match_count"] for r in alert_results)
                log.info(
                    "Auto-evaluated %d alert rules — %d total matches across %d providers",
                    len(enabled_rules), total_matches, len(all_providers),
                )
                log_action(
                    "alert_evaluated", "system", "auto",
                    details={"rules_evaluated": len(enabled_rules), "total_matches": total_matches,
                             "provider_count": len(all_providers), "trigger": "post_scan"},
                )
        except Exception as alert_exc:
            log.warning("Auto-evaluate alert rules failed: %s", alert_exc)
...
async def lifespan(app: FastAPI):
    ...
    # Load PECOS (Medicare enrollment + reassignment) — try cache first, download if missing
    if not load_pecos_from_disk():
        asyncio.create_task(download_pecos_data())
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5200", "http://localhost:5173", "http://localhost:3000", "http://localhost:8001", "https://medicaid-inspector.web.app", "https://medicaid-inspector.firebaseapp.com"],
...
async def rescore_cached_providers():
    ...
    for p in providers:
        ...
        # New signals — servicing/bene data not available on rescore (no Parquet re-query)
        s18 = ghost_employee_billing(npi, [])
        s19 = servicing_npi_concentration(npi, [])
        s20 = beneficiary_sharing_anomaly(npi, [])
        s21 = pecos_enrollment_gap(npi)
        s22 = reassignment_chain_depth(npi)
    ...
    try:
        ...
        for batch_i, i in enumerate(range(0, n_candidates, BATCH)):
            ...
            for row in batch:
                ...
                s16 = new_provider_explosion(row)
                s17 = geographic_impossibility(row)
                # Servicing/bene signals not available in smart scan (no extra queries)
                s18 = ghost_employee_billing(npi, [])
                s19 = servicing_npi_concentration(npi, [])
                s20 = beneficiary_sharing_anomaly(npi, [])
                s21 = pecos_enrollment_gap(npi)
