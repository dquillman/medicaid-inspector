          ...
          <div className="rounded-2xl border border-gray-800 bg-gray-900/50 p-1 shadow-2xl shadow-black/50">
            <div className="rounded-xl bg-gray-950 border border-gray-800 p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-900 rounded-lg p-4 border border-gray-800 h-48 flex flex-col">
                  <p className="text-xs text-gray-600 mb-2">State Heatmap</p>
                  <svg viewBox="0 172 1024 680" className="flex-1 w-full min-h-0" preserveAspectRatio="xMidYMid meet">
                    <defs>
                      <clipPath id="usOutline">
                        <path d={US_MAP_PATH_MAIN} transform={US_MAP_TRANSFORM} />
                        <path d={US_MAP_PATH_ISLE} transform={US_MAP_TRANSFORM} />
                      </clipPath>
                      <filter id="heatBlur" x="-50%" y="-50%" width="200%" height="200%">
                        <feGaussianBlur stdDeviation="18" />
                      </filter>
                    </defs>
                    <g transform={US_MAP_TRANSFORM} fill="#1f2937">
                      <path d={US_MAP_PATH_MAIN} />
                      <path d={US_MAP_PATH_ISLE} />
                    </g>
                    <g clipPath="url(#usOutline)" filter="url(#heatBlur)">
                      {HEAT_SPOTS.map((s, i) => (
                        <circle key={i} cx={s.cx} cy={s.cy} r={s.r} fill={s.fill} opacity={s.opacity} />
                      ))}
                    </g>
                    <g transform={US_MAP_TRANSFORM} fill="none" stroke="#ef4444" strokeOpacity="0.3" strokeWidth="8">
                      <path d={US_MAP_PATH_MAIN} />
                      <path d={US_MAP_PATH_ISLE} />
                    </g>
                  </svg>
                ...
                <div className="bg-gray-900 rounded-lg p-4 border border-gray-800 h-48 flex flex-col">
                  <p className="text-xs text-gray-600 mb-2">Fraud Signal Distribution</p>
                  <div className="flex-1 flex flex-col justify-between gap-1">
                    {SIGNAL_DISTRIBUTION.map(s => (
                      <div key={s.label} className="flex items-center gap-2">
                        <span className="text-[10px] text-gray-600 w-20 truncate">{s.label}</span>
