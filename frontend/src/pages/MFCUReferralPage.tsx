...
export default function MFCUReferralPage() {
  ...
  // THE referral narrative — generated by the same backend generator the OIG
  // tip uses (destination='mfcu' swaps only the routing header/footer). The
  // reports are ALWAYS a referral narrative and it IS the submission: both
  // intakes are text-only forms, so this carries the full evidence — street
  // address from live NPPES, dollars + per-beneficiary intensity, federal
  // exclusion check, and every signal with its regulatory citation.
  const { data: referralNarrative } = useQuery({
    queryKey: ['referral-narrative', npi, 'mfcu'],
    queryFn: () => api.oigTip(npi!, 'mfcu'),
    enabled: !!npi,
    staleTime: 5 * 60_000,
  })
